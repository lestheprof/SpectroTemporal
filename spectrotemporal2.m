function weightarray = spectrotemporal2(SD,filelistfile, varargin)
%spectrotemporal2 finds the weights that arise from this set of sound
% This version trains the neurons one at a time
...
%  this version started 7 May 2019
...
M = 1 ; % number of LIF neurons: 1 at a time here
...
weightnormtype = 1; % 1 is mean 0, norm = weight norm; 2 is all nonneg.
weightarray = [] ;
...
while(i<=size(varargin,2))
    switch lower(varargin{i})
        ...
        case 'weightssupplied'
            ...
            i=i+1 ;
        case 'weightnorm'
            weightnorm = varargin{i+1}; % -weightmaxval to weightmaxval is weight range if weightnotmtype is 1
            i=i+1 ; % but 0 to wieghtmaxval if weightnormtype  == 2
        case 'weightnormtype'
        ...
        otherwise
            error('spectrotemporal2: Unknown argument %s given',varargin{i});
    end
    i=i+1 ;
end
...
if ~(useabs || useonset || useoffset)
    ...
    disp('spectrotemporal2: no input sets selected') ;
    return  ;
end
...
if ~(weightssupplied) % default
    ...
    for neuron = 1: M
        ...
        if weightnormtype == 2 % nonnegative weights
            % initialise weights randomly within some range
            weightarray = random('uniform', 0, +1, [M, N*ranges, K]) ;
            for neuron = 1: M
                % weightarray(neuron, :, :) = weightarray(neuron, :, :) - mean(weightarray(neuron, :, :), 'all') ;
                weightarray(neuron, :, :) = weightnorm * (weightarray(neuron, :, :)/norm(squeeze(weightarray(neuron, :, :)))) ;
            end
    ...
    if ~(isequal(size(weightarray), [M, N*ranges, K]))
        disp(['spectrotemporal2: supplied weight array and size of bandpass filter, neural array, time lapse do not agree', ...
            num2str(size(weightarray,1)), ' ' , num2str(size(weightarray,2)), ' ', num2str(size(weightarray,3))]) ;
        return ; % fatal error
    end
end
if ~firstneuron
    % check that the existing weights are of a valid size. Should be no of
    % existing neurons by N*ranges, by K
    [n_existing inputbands ltimesteps] = size(existingweights) ;
    if ~(isequal([N*ranges, K], [inputbands ltimesteps]))
        disp(['spectrotemporal2: supplied existing weight array and size of bandpass filter, time lapse do not agree', ...
            num2str(inputbands), num2str(ltimesteps)])  ;
        return ; % fatal error
    end
end
...
% starting with the first neuron, train up, then for next neuron subtract
% the projection of the first neuron, etc.
...
for i = 1:nooffiles
    ...
    for ts = 1:(size(r_absSig, 2) - K) % for each possible time step - without overlapping the end of the signal
        ...
        % is timesteps by B;
        %   existingweights is used to calculate the activities of each of
        %   the n_existing neurons, and then calculate the decrement in
        %   input by feeding these activities (negartively) back to the input
        % output is M by 1
        %
        newLIFactivity = zeros([M 1]) ;
        LIFfiring = zeros([1 M])  ; % nothing firing
        % if this is not the first neuron, we need to adjust the values in
        % r_absSig, r_onset_signal and r_offsetsignal
        if ~firstneuron
            if useabs
                n_absSig = r_absSig(:, ts : ts+K-1) ;
        ...
        else % not first neuron, so train on input less feedback
            for tt = 1:K
                if useabs
            ...
            if firstneuron
                for nno = 1:M
                    if LIFfiring(nno)
                        ...
                        else if weightnormtype == 2
                               weightarray(nno, :, :) = max(0, weightarray(nno, :, :)) ; 
                               weightarray(nno, :, :) = weightnorm * (weightarray(nno, :, :)/norm(squeeze(weightarray(nno, :, :)))) ; % norm = weightnorm
                            end
                        end
                    else
                        % neuron did not fire
                        % we should not get to here if M is 1
                        disp('spatiotemporal2: adapting non-firing neuron. Should not happn if M is 1') ;
                        if useabs
